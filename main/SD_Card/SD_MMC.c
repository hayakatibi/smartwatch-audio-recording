...
bool SD_Init(sdmmc_card_t **out_card)
{
    esp_err_t ret;
    sdmmc_card_t *card = NULL; // Local single pointer
    ...
    *out_card = card;
    // Add to your code after mounting:
    // ESP_LOGI(SD_TAG, "Testing file system");
    // FILE *f = fopen("/sdcard/test.txt", "w");
    // if (f)
    // {
    //     fprintf(f, "Test %lu\n", esp_log_timestamp());
    //     fclose(f);
    //     ESP_LOGI(SD_TAG, "Write test succeeded");
    // }
    // else
    // {
    //     ESP_LOGE(SD_TAG, "Write test failed");
    // }
    return true;
}
